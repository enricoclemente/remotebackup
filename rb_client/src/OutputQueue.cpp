...
void OutputQueue::add_file_operation(const std::string& path, FileCommand command, std::time_t last_write_time) {
    std::lock_guard lg(m);

    std::shared_ptr<FileOperation> fo =
            std::shared_ptr<FileOperation>(new FileOperation{path, command, last_write_time, id_counter});
...
void OutputQueue::remove_file_operation(int id) {
    std::unique_lock ul(m);
    ...
    // TODO nel caso in cui non trovo l'elemento con l'id passato
    //  lancio eccezione o faccio ritornare un bool dalla funzione?
}
...
FileOperation::FileOperation(std::string path, FileCommand command, std::time_t last_write_time, int id) :
    path(std::move(path)), command(command), last_write_time(last_write_time), id(id) {
    processing = false;
    abort = false;
}
...
std::time_t FileOperation::get_last_write_time() const {
